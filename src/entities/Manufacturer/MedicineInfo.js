...
export default function MedicineInfo(props) {
  ...
  const [medicineAddress] = useState(query.address || (props.match && props.match.params && props.match.params.id) || "0xBBB0000000000000000000000000000000000001");
  ...
  async function getMedicineData() {
    try {
      ...
      setWholesaler(data[8] || "0x5234567890123456789012345678901234567890");
      setMedicineData(data);
      setActiveStep(step);
    } catch (err) {
      console.warn("Could not load manufacturer medicine data:", err);
    } finally {
      isLoading(false);
    }
  }
    ...
    supplyChain.methods
      ...
      .once("receipt", async (receipt) => {
        ...
        transactions.methods
          ...
          .send({ from: account })
          .once("receipt", () => {
            alert("Package dispatched to Wholesaler successfully!");
          });
      });
  }
  ...
  return (
    <div className={classes.card}>
      <h2 style={{ color: "#38bdf8", marginTop: 0 }}>Manufactured Medicine Details</h2>
      <FetchAPI />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 200px", gap: "24px", alignItems: "start" }}>
        <div>
          ...
          <p><b>Raw Materials Used:</b> {medicineData ? medicineData[2] : "N/A"}</p>
          <p><b>Batch Quantity:</b> {medicineData ? medicineData[3] : "N/A"} units</p>
          <p><b>Transporter:</b> {medicineData ? (Array.isArray(medicineData[4]) ? medicineData[4].join(", ") : medicineData[4]) : "N/A"}</p>
          <p><b>Wholesaler:</b> {wholesaler}</p>
          <p><b>Distributor:</b> {medicineData ? medicineData[5] : "N/A"}</p>
        ...
        <div style={{ textAlign: "center", background: "rgba(255,255,255,0.05)", padding: "16px", borderRadius: "12px" }}>
          <QRCodeSVG value={`${window.location.origin}/manufacturer/view-medicine/${medicineAddress}`} size={150} level="H" includeMargin={true} />
          <p style={{ fontSize: "0.8rem", color: "#94a3b8", marginTop: "8px" }}>Scan to Verify Batch Integrity</p>
        </div>
      </div>
