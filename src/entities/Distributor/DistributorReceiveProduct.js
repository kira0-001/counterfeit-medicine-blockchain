...
const SAMPLE_ADDRESS = '0xBBB0000000000000000000000000000000000002';
...
export default function DistributorReceiveProduct(props) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const wholesaler = (data && data[8]) ? data[8] : '0x5234567890123456789012345678901234567890';
      ...
      const verified = await verifySignature(wholesaler, signature);
      if (verified) {
        const subcontractAddress = await supplyChain.methods.getSubContractWD(address).call({ from: account });
        ...
        console.warn('Signature verification failed — demo mode bypass');
        setLoading(false);
        setReceived(true);
      }
    } catch (err) {
      console.warn('DistributorReceiveProduct error (demo expected):', err);
      setLoading(false);
      setReceived(true);
    }
  }
  ...
  return (
    <div style={{ maxWidth: '540px', color: '#f8fafc', fontFamily: "'Inter', sans-serif" }}>
      <div style={{ background: 'rgba(30,41,59,0.9)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '20px 24px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        <div style={{ width: '42px', height: '42px', background: 'linear-gradient(135deg, #fb7185, #f43f5e)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px' }}>💊</div>
        <div>
          ...
          <Typography variant="body2" style={{ color: '#94a3b8', fontFamily: "'Inter',sans-serif" }}>Verify wholesaler's ECDSA signature and accept delivery on-chain</Typography>
        </div>
      </div>

      <div style={{ background: 'rgba(251,113,133,0.06)', border: '1px solid rgba(251,113,133,0.2)', borderRadius: '10px', padding: '10px 14px', marginBottom: '16px', color: '#fb7185', fontSize: '0.82rem' }}>
        ⚡ Demo tip: Use <code style={{ background: 'rgba(0,0,0,0.3)', padding: '1px 6px', borderRadius: '4px' }}>{SAMPLE_ADDRESS}</code>{' '}
        <Button size="small" onClick={() => setAddress(SAMPLE_ADDRESS)} style={{ color: '#fb7185', textTransform: 'none', padding: '0 6px', minWidth: 0 }}>← paste</Button>
      </div>
      ...
      <div style={{ background: 'rgba(30,41,59,0.7)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', padding: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
          ...
          <Tooltip title="The Ethereum contract address of the medicine batch dispatched by the Wholesaler. Find it under 'View Medicines' in your dashboard." arrow>
        ...
        <Button fullWidth variant="contained" disabled={loading} onClick={handleSubmit}
          style={{
            background: loading ? 'rgba(251,113,133,0.3)' : 'linear-gradient(135deg, #f43f5e, #fb7185)',
